...
|----------|----------|---------|-------------|
| `DATABASE_URL` | No | `sqlite:////data/margin.db` | SQLAlchemy database URL |
...
The SQLite database lives on the `margin_data` Docker volume mounted at `/data` — it survives deploys and container restarts. WAL mode is enabled automatically (see `margin/db.py`), so concurrent readers never block and concurrent writes queue via `busy_timeout`.

Migrations run automatically on container start (`bin/docker-entrypoint`).

To back up: `kamal server exec "docker run --rm -v margin_data:/data -v /tmp:/backup alpine tar czf /backup/db.tar.gz /data"` and copy the archive off the server.
