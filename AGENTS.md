...
| API | FastAPI app in `margin/api.py`, mounted via `rx.App(api_transformer=api)` |
| Database | SQLite with WAL mode — SQLModel models, Alembic migrations |
| Auth | Passwordless magic links (no auth library) — ~100 lines in `margin/auth.py` |
...
- Business logic lives in plain functions under `margin/` (see `auth.py`) — NOT in Reflex state, NOT in service classes
...
- Models subclass `sqlmodel.SQLModel` with `table=True` in `margin/models.py`, with an explicit integer `id` primary key
...
- SQLite PRAGMAs (WAL, busy_timeout, foreign_keys) are set in `margin/db.py` — importing the module registers them
...
- Pages are functions returning `rx.Component` in `margin/pages/` — one page per file
...
- JSON endpoints for external clients go in `margin/api.py` (FastAPI) — they share the backend port and are proxied under the same origin in production
...
- `margin/auth.py` is the single source of truth
...
| Avoid | Use instead |
|-------|------------|
| Auth libraries (authlib for login, fastapi-users) | `margin/auth.py` (already in this repo) |
| Service classes / repositories | Plain functions taking a `sqlmodel.Session` |
| pydantic-settings | `os.getenv` in `margin/settings.py` |
...
```
margin/
├── margin.py    # App entry — rx.App(api_transformer=api)
